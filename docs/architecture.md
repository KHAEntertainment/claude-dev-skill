...
**Status: superseded by [ADR-010](#adr-010--confirmed-project-intent-and-pre-write-verification), 2026-09-11.** The following records the rejected historical proposal, not current implementation or planned work. Its credential layer, liveness preflight, SSH rewrite, and arbitrary-command containment target do not apply.

- **Issues**: [#19](https://github.com/KHAEntertainment/claude-dev-skill/issues/19) (repository identity, restarted after nine rounds); dogfooding finding F-1.

- **Decision**: Guaranteeing that no GitHub write reaches a non-canonical repository is treated as a question about what the run is *able* to do, not a question about whether each write was checked first. Three layers carry the guarantee. Two ship in this round: an enforcement-liveness preflight that positively verifies enforcement is installed and active before the workflow proceeds, and a honeypot corpus that proves behaviourally that no fixture scenario writes where it should not. The third and strongest — a credential scoped to the canonical repository, so that a misroute fails at GitHub's API rather than at a local check — is **deferred**, and its absence is the accepted risk recorded below. Explicit `--repo OWNER/REPO` scoping on every command remains throughout, but as defence in depth rather than as the guarantee.
...
- **Accepted risk**: until the scoped credential lands, every layer that ships is client-side, and each is one `git config --unset` or `PATH` edit away from being gone. A misrouted write therefore still fails at a local check rather than at GitHub's API, which is exactly the property the nine failed rounds showed to be insufficient. The liveness preflight narrows this — it makes disarmament detectable, and it hard-stops the workflow rather than the individual write — but detection is not prevention, and a run that never reaches the preflight is unprotected. This risk is accepted for one round on the grounds that a GitHub App installation token cannot be minted programmatically and imposes a real setup cost on third-party installs; it is not accepted indefinitely.

## ADR-009 — The read-only AST guard is a CI-time authorship lint, not a security control

- **Issues**: [#17](https://github.com/KHAEntertainment/claude-dev-skill/issues/17) (residual bypass, documented rather than patched); arises from the [#19](https://github.com/KHAEntertainment/claude-dev-skill/issues/19) review rounds.

- **Decision**: The AST guard in `scripts/validate_skill.py` is reclassified as a lint that constrains what this repository's own authors ship. It is not a runtime security boundary, and it is not the control that keeps a misrouted write from landing. The supported pre-write verification in ADR-010 is exercised by behavioral tests, including physical local-repository push fixtures; the rejected ADR-008 credential layer is not a control. **Deferred to #17:** separately named guard-too-strict and guard-too-loose CI checks. The split is not a v2.1.0 release prerequisite.
- **Decision time**: 2026-09-05
...
- **Consequence**: Guard work is capped. Effort moves to broadening behavioural coverage, which catches what static analysis structurally cannot and is spelling-agnostic. If the guard is revisited, the preferred direction is restricting the resolver's module surface — asserting it imports nothing outside an allowlist and binds no unexpected module-level names — rather than enumerating call spellings. **Deferred to [#38](https://github.com/KHAEntertainment/claude-dev-skill/issues/38):** recording the reclassification in `skills/dev/backends/contract.md`, where the delegated lanes read it. This is not a v2.1.0 release prerequisite. That placement is deliberate rather than incidental: it exists so a future round does not re-litigate the question and spend another five review cycles on it.

## ADR-010 — Confirmed project intent and pre-write verification

- **Decision time**: 2026-09-11; records the approved scope implemented in [PR #36](https://github.com/KHAEntertainment/claude-dev-skill/pull/36), superseding ADR-008.
- **Decision**: A local, Git-excluded `.dev.json` stores confirmed nonsecret GitHub account and repository targets. First-use setup verifies candidates and asks for missing intent. Independent-fork and upstream-contributor presets initialize explicit push/Issue and PR targets; upstream is never inferred as a write target.
...
## ADR-012 — Child harnesses receive handoff context, never /dev variants

- **Issues**: #58 (this ADR + checklist); #38 (ADR-009 guard reclassification, coupled — same file).
- **Decision time**: 2026-09-13.
- **Decision**: Codex CLI, OpenCode, Cursor-class, and any other Claude-class-or-similar receive-capable harness serve as Traycer-managed child agents by receiving the full role prompt plus the provider-neutral assignment envelope. There are no `/dev` variants and no forked SOPs. The contract already asserts and pins this: `skills/dev/backends/traycer.md:69-71` (children execute provider-neutral assignments) and `skills/dev/backends/contract.md:33-37` (the envelope). Position B's real concerns — capability gating and routing answers for model variance — are answered by the four-item capability checklist (Deliverable B), not by variants.
- **Routing answer**: Model variance is the routing concern, not the contract concern. A child harness that satisfies the capability checklist receives the assignment; one that does not is recorded `incomplete` at the gate. The remedy for a gate failure is always a different route (a child that satisfies the checklist, or the lead dispatching directly when no child qualifies) — never a trimmed prompt, never a relaxed checklist. This rule is pinned both in the checklist subsection (Deliverable B) and in the doc-assertion tests (Deliverable D).
- **Escape hatch — native non-Claude lead**: A separate, thin distribution for a native non-Claude lead is permitted, but it must implement the same envelope and capability gate rather than reuse the Claude-Code-specific paths. Reusing Claude-Code-only paths would recreate the rejected competing-SOP problem and the Ponytail #43 config-leak problem. No implementation work is scoped here; this ADR records the decision and bounds the escape hatch.
- **Consequence**: A new dispatched lane is `incomplete` if any of the four checklist items fails. The lead's ledger field for that lane records which item failed and which alternate route was selected. The capability checklist subsection in `skills/dev/backends/contract.md` (Deliverable B) is the canonical statement; this ADR is the rationale. The experiment issue (Issue #43 mirror — separate, out of v2.1.1 scope) will exercise Kimi CLI and Qwen Code to confirm the checklist gates them correctly.
- **Pinned tokens**: the sentence "Child harnesses receive handoff context, never /dev variants" must appear verbatim in this ADR (doc-assertion test pins it).
