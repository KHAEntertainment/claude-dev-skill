...
- **Operational prerequisite**: `claude plugin` fetches marketplace and plugin sources over **SSH**. An authenticated `gh` reporting `Git operations protocol: https` does not satisfy this — the transports are independent, and a machine with working HTTPS access can still fail the plugin install with `Permission denied (publickey)`. Verified on a machine where `git ls-remote git@github.com:...` failed while the HTTPS equivalent succeeded. The README documents this as a requirement and as a troubleshooting step, and `docs/RELEASING.md` step 5 prechecks it.
