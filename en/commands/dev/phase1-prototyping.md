...
prompt: <paste the full content of worker-prototype-frontend.md or worker-prototype-backend.md>
...
- By default `prototypes/` is in `.gitignore` (see the gitignore guidance in `PROJECT_CONTEXT_TEMPLATE.md`). The user can selectively `git add` key prototypes.
