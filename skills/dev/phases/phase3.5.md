...
6. Record `rtk git status --short` before and after the lane. Any tracked QA change fails the lane and makes the adapter result incomplete.
...
Treat QA agents as one-shot, read-only lanes. They report evidence; they do not approve, merge, or modify code. A QA agent ID must differ from the implementation worker ID. A new push invalidates its result.
