...
Keep `consecutive_rate_limits` per reviewer current while retrying; a non-rate-limit response resets it to 0, and reaching 3 triggers the rate-limit breakpoint in `${CLAUDE_SKILL_DIR}/phases/external-review.md`. Each `external_review_substitutions` entry records `external_reviewer_unavailable: rate_limited`, the substituted reviewer, the `consecutive_rate_limits` count, the substitute identity and model family, the reviewed `headRefOid`, the substitute's verdict, and the PR comment URL. A substitution is a completed review, not an `approved_bypasses` or `review_debt` entry.
