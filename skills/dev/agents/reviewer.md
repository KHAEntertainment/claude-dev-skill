...
- Before the first PR read, require exit 0 from `rtk proxy python3 "${CLAUDE_SKILL_DIR}/scripts/resolve_repository.py" --operation pr --target [pr-repository]`; scope PR reads with `--repo github.com/[pr-repository]`.
- Before the first assigned Issue read, require exit 0 from `rtk proxy python3 "${CLAUDE_SKILL_DIR}/scripts/resolve_repository.py" --operation issue --target [issue-repository]`; use `--allow-target-override` only for an explicitly assigned existing Issue in a different repository, confirmed in the ledger. Scope Issue reads with `--repo github.com/[issue-repository]`. Never substitute the PR repository or infer it from the Issue number. Use RTK-first commands and deep-read only the assigned PR/Issue and directly relevant files.
