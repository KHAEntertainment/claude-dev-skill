...
- **The verdict never varies; the recorded cause does.** Record `report_back: incomplete` with cause `absent` (no reply correlated to the recorded response ID), `malformed` (a complete reply arrived without every section), or `truncated` (the read ended without the transport declaring the reply complete). The verdict is identical because all three leave the lane unverified. The cause differs because the remedy does: `absent` questions whether the lane is alive at all and is answered by inspecting live state before re-requesting or replacing the lane; `malformed` comes from a demonstrably alive lane and is answered by re-requesting the shape at the recorded response ID; `truncated` is re-read before it is re-requested, because the missing sections may already exist upstream of the cut.
...
- **`absent` is classified first, before any shape or paging classification runs.** The read still happens first — nothing can be known to be absent without looking. What comes first is the *classification*: once the read ends, ask whether a reply correlated to the recorded response ID was observed at all, and if none was, record cause `absent` and stop. The shape and paging branches below never run on an empty read. This precedence is part of the taxonomy rather than each adapter's discretion, because both branches misclassify silence when they run first: a surface that returns an empty message as complete reaches the shape branch and records `malformed` — seven sections missing from nothing — while a re-read of an empty surface stalls and records `truncated`, a cut in a reply that never existed. Each sends the lead to the wrong remedy. Only `absent` says to inspect whether the lane is alive at all. **An empty read is never evidence of a reply's shape or of its transport.**
